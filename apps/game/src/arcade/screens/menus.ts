...
import { GENIUSES, WING_ORDER, WINGS, geniusesInWing, getGenius, type Genius } from '@vk/core';
...
import { startCouncil, startExhibition, toMode, toTitle } from '../flow.js';
...
export class ModeScreen implements Screen {
  ...
  update(game: Game): void {
    ...
    if (inp.pressed('up') || inp.pressed('down') || inp.pressed('left') || inp.pressed('right')) {
    ...
    if (inp.pressed('back')) {
      ...
      game.audio.play('confirm');
      if (MODES[this.cursor]!.id === 'exhibition') game.go(new SelectScreen());
      else startCouncil(game);
    }
  }
  ...
  render(g: Gfx): void {
    ...
    MODES.forEach((m, i) => {
      ...
      frame(g, 30, y, 260, 66, on ? '#2a1a30' : '#15121c', on ? (blink(this.t, 8) ? C.gold : C.goldDark) : C.panelEdge);
      blit(g, bigText(m.label, 3, on ? '#ffffff' : '#9a90a8', on ? '#f2c14e' : '#5a5068'), 44, y + 8, 'left');
      m.desc.forEach((d, k) => text(g, d, 46, y + 32 + k * 9, { color: on ? C.white : C.grey }));
      if (on) text(g, '▶', 36, y + 14, { color: C.gold, scale: 2 });
      // Mini fighters.
      const wings = i === 0 ? (['questioners', 'artists'] as const) : (['strategists', 'builders'] as const);
      drawFighter(g, wings[0], on ? 'jab1' : 'idle0', 232, y + 62, 1, 'normal', 0, 1);
      drawFighter(g, wings[1], on ? 'block' : 'idle0', 266, y + 62, -1, 'normal', 0, 1);
    });
...
export class SelectScreen implements Screen {
  ...
  render(g: Gfx): void {
    ...
    list.slice(start, start + LIST_ROWS).forEach((gn, k) => {
      ...
      const name = gn.name.length > 30 ? `${gn.name.slice(0, 29)}.` : gn.name;
      text(g, name, 97, y, { color: on ? '#ffffff' : C.grey, shadow: null });
      if (gn.living) text(g, 'L', 223, y, { align: 'right', color: C.cyan, shadow: null });
    });
    ...
    wrap(`METHOD: ${focus.method}`, W - 22).slice(0, 3).forEach((l, i) => text(g, l, 10, 171 + i * 8, { color: C.white }));
    text(g, 'SIDE A ARGUES: FREE WILL IS AN ILLUSION - SIDE B: IT IS REAL', 10, 200, { color: C.dim });
