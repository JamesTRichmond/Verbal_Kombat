    ...
    <div class="screen">
      <div class="report">
        ...
        <table>
          ...
          ${row('XP earned', xpA, xpB)}
        </table>
        <button id="btn-study">STUDY THE TRANSCRIPT</button>
      </div>
    </div>`);
...
function enableScrub(): void {
  if (!replay) return;
  scrub.disabled = false;
  scrub.max = String(replay.entries.length);
  scrub.value = '0';
  scrubLabel.textContent = 'scrub 0/' + replay.entries.length;
  scrub.oninput = () => {
    const i = Number(scrub.value);
    scrubLabel.textContent = `scrub ${i}/${replay!.entries.length}`;
    // simplified scrub — full implementation already on main for report flow
  };
}
