...
1. **Argument select** — 8 category chips; picking one shows 3 pre-written questions from `data/topics.json`, plus a custom-question field. The custom field lives inside the selected category, so a custom question inherits that category's dialogue line banks — there is no category-less custom path.
...
3. **Location select** — 2 arenas (say, The Forum and The Studio). Release 1 scope: distinct palette/parallax and one scripted environmental event each (e.g. the Forum's echo doubles the next landed point's dialogue weight). Cosmetic-plus, not full tactics yet. All audio — crowd, announcer, effects — is deferred with sound in §3's later-milestone list; an arena satisfies R1 without it.
...
2. **Data schemas + seed content** — `topics.json` (8 categories × 3 questions), `fighters.json` (4 fighters: stats, style, special, line-bank keys, CPU behavior pattern per D10), `locations.json` (2 arenas, each with its scripted environmental event), `lines/` template banks. AC: schemas documented in `data/README`; content loads with the existing dataLoader fallback pattern; custom topic input is escaped/XSS-safe.
3. **Screen flow state machine** — Argument → Fighter → Location → Fight → Verdict, keyboard + mouse navigable, back-navigation supported. AC: can traverse the full flow with placeholder screens; deep state (selections) survives to the verdict; every interactive control on every screen shows a visible focus indicator (keyboard-first per §4) — this criterion carries into the selection, fight, and verdict screen issues.
...
6. **Location selection screen** — 2 arenas with visual identity. AC: arena changes fight backdrop/palette; chosen arena recorded in ledger for judging; each arena's scripted environmental event fires during a match, emits a ledger event, and applies its effect (e.g. the Forum echo's dialogue-weight boost).
7. **Combat core** (largest issue — consider splitting movement/attacks/blocking) — entities, input, hitboxes, HP, knockback, block, round timer, KO. AC: two fighters can fight to KO or time-out at 60fps; the CPU opponent plays its fighter's scripted behavior pattern — aggression curve and punish windows per D10 — so a full single-player match is playable; all combat events emit to the ledger with timestamps; deterministic under a seeded RNG for tests.
